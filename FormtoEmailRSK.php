...
$continue = "http://www.jdmi.com/thanksforresponding.html";
...
if(substr_count($_REQUEST['email'],"@") != 1 || stristr($_REQUEST['email']," ") || stristr($_REQUEST['email'],"\\") || stristr($_REQUEST['email'],":")){$errors[] = "Sorry, Your Email address is Invalid";}else{$exploded_email = explode("@",$_REQUEST['email']);if(empty($exploded_email[0]) || strlen($exploded_email[0]) > 64 || empty($exploded_email[1])){$errors[] = "Email address is invalid";}else{if(substr_count($exploded_email[1],".") == 0){$errors[] = "Email address is invalid";}else{$exploded_domain = explode(".",$exploded_email[1]);if(in_array("",$exploded_domain)){$errors[] = "Sorry, Your Email address is Invalid";}else{foreach($exploded_domain as $value){if(strlen($value) > 63 || !preg_match('/^[a-z0-9-]+$/i',$value)){$errors[] = "Sorry, Your Email address is Invalid"; break;}}}}}}
...
$message = $message . PHP_EOL.PHP_EOL."-- ".PHP_EOL."Sent from JDMI App or Website";

$message = stripslashes($message);

$subject = "RS Kerr Warehouse";
...
$URL="http://www.jdmi.com/thanksforresponding.html"; 
